...
### 1.2 운영 프로토콜 (Operating Protocol)
1.  **만장일치제**: 기술 부채 도입, 보안 정책 변경은 6인의 만장일치 필요.
2.  **No Summary Rule (Strict)**:
    - AI는 절대 페르소나의 의견을 요약하거나 의역하지 않는다.
    - 반드시 **"인용구(Blockquote)"**와 **"전체 문장(Full Sentence)"**을 사용하여 원문 그대로 출력한다.
    - *Bad*: "PM은 찬성했습니다."
    - *Good*: "PM: '이 변경사항은 로드맵의 가시성을 높이므로 즉시 승인합니다.'"
3.  **품질 게이트(Quality Gate)**: 각 단계별(Phase) 종료 시 Council Review 문서 필수 작성.
